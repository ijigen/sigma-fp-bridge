...
**1.** Download `sigma-fp-bridge-v0.1.1-macos-arm64` from [**Releases**](../../releases).
...
```bash
cd ~/Downloads
chmod +x sigma-fp-bridge-v0.1.1-macos-arm64
xattr -d com.apple.quarantine sigma-fp-bridge-v0.1.1-macos-arm64
sudo ./sigma-fp-bridge-v0.1.1-macos-arm64
```
...
| Port 1025 is taken | `SIGMA_BRIDGE_PORT=9000 sudo -E ./sigma-fp-bridge-v0.1.1-macos-arm64`, then open `http://localhost:9000/` |
...
Output is `dist/sigma-fp-bridge-v<version>-<os>-<arch>`, one file, about 10 MB.
The version comes from `VERSION` in `mac_bridge_server.py`; the os and arch come
from the machine you build on, not from a constant. It is what gets
