...
def partial_movie(cam, offset: int, length: int) -> bytes:
    ...
    **重啟 bridge 沒有用** —— 那已經是新的 PTP session 了，狀態仍在。只有把
    ...
    尚未驗證的假設：**第一個參數可能是「這次拍攝裡的第幾個檔案」。**
    parse_movie_file_info 解出來的結構有 FileCount 和一張偏移表，但 MOV 是
    單檔，FileCount 一直都是 1，那張表從沒被用到。CinemaDNG 錄的是一整包
    逐格 .dng，如果 FileCount 會 > 1，那個參數就有了意義。

    先前判定「第一個參數不是索引」的依據是傳 1、2 只拿到 122,868 的拒絕
    回應 —— 但那是在單檔 MOV 上測的，當時只有檔案 0 存在，拒絕是應該的。
    那個結論的適用範圍比我當時寫的窄。

    另外，**只有資料庫索引 0 的影片讀得到，而且是「索引 0」不是「head」。**
    這兩者長期分不開（head 每次檢查時都剛好是 0），後來用兩段不同長度的影片
    分開驗證：
      - 項目 0（31,464,592）與項目 1（40,311,528）都在，讀取在 31,464,592
        被拒 —— 服務的是項目 0
      - 只釋放項目 0（head 前進到 1，項目 1 仍在）之後再讀 → USBTimeoutError

    所以它綁的是位置 0 本身。head 前進之後那個位置沒有東西，就變成會讓相機
    掛掉的條件。逐筆釋放來連續下載的想法行不通。

    要下載下一段影片，只能 release + acquire 讓資料庫歸零，再錄一段。

    ⚠️ **不要把 DataGroupMovie 的 tag 10 設成 0。**（它有可能根本不是故障，
    而是「關閉主機傳輸」這個功能本身 —— tag 10 gate 住的 11/12/13/14 對影像
    管線與 HDMI 輸出都毫無影響，唯一可觀測的效果就是傳輸停止。不過設回 1
    並不會恢復傳輸，這點跟單純的開關不符。）那是一個主開關 —— 關掉之後
    能力 tag 110/112/113/114 全部變成不可設定，而且相機會停止服務影片傳輸：
    錄影照常、檔案照常寫進記憶卡、MovieFileInfo 照常回報正常大小，只有 0x9037
    改回 122,868。它實際控制什麼還不知道。

    復原方式：**只有把相機斷電重開**。切 STILL、重錄、release+acquire、重啟
    bridge 都試過，沒有一個有效。

    這段結論我改錯過兩次，方式一模一樣，所以把教訓記下來而不只是記結論：
    兩次都是我看到「傳輸恢復了」，就從自己剛做過的動作裡挑一個當原因，
    寫成「會自行恢復」。實際上那兩次都是使用者把相機斷電重開了 —— 而那件事
    不在我的觀測範圍內。

    在一個我看不到全部輸入的系統裡，「它好了」不能推論出「是我做的那件事讓它
    好的」。要嘛先確認期間沒有其他人動過，要嘛就只寫「觀察到恢復，原因不明」。

    另外，觸發條件可能是「寫入 tag 10」本身而不是寫入的值 —— 有一次只是把它
    寫成原本就有的 1，之後傳輸就壞了。最省事的規則：**不要碰 tag 10。**
    """
    from construct import Container
    ...
    while len(out) < movie.size:
        ...
        if len(got) != want:
            ...
            raise RecordingError(
                ...
                "相機停止服務影片傳輸。**只有把相機斷電重開有效** —— 切 STILL、"
                "重錄、release+acquire、重啟 bridge 全都試過，沒有一個有用。")
        out += got
