...
#:
#: 實測過的行為：
#:   - 256 / 512 / 1024 / 4096 → 回傳真實影片資料（用重疊比對驗證：從 N 讀到的
#:     位元組等於從 0 讀的第 N 個之後）
#:   - 65536 以上 → 回傳「要求的長度」但內容是垃圾，而且會永久毀掉這個
#:     PTP session：之後每個請求都回同一坨 122,868 bytes 的殘留緩衝區
#:     （內容是上一個指令的回應加上舊記憶體），要重啟 bridge 才會好
#:
#: 曾經設成 4 MB，理由是探測時「1/4/16 MB 都回傳了剛好的位元組數」—— 那次
#: 只比對長度沒看內容，於是把一個根本不傳影片的尺寸當成驗證過，拼出一個大小
#: 完全正確、內容全錯的 27 MB 檔案。長度不是證據。
#:
#: 8192 / 16384 / 32768 還沒驗過。要往上調就得逐級用內容驗證，而且每失敗一次
#: 就得重啟一次 session。
